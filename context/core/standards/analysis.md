...
- Rails complexity (method length, concern organization)
- Code duplication (extract to concerns)
- Rails test coverage (model, controller, system tests)
- Documentation completeness (Rails-specific patterns)
- Rails naming consistency (conventions followed)
- Error handling patterns (Rails validation, rescue_from)

### Architecture Analysis
- Rails module dependencies (models, controllers, concerns)
- Coupling and cohesion (concern boundaries)
- Separation of concerns (MVC, business logic in models)
- Rails scalability (database design, background jobs)
...
### Pattern Discovery
- Search for Rails pattern implementations
- Identify 37signals/Basecamp pattern usage
- Document Rails conventions followed
- Note inconsistencies with Rails patterns
- Recommend Rails standardization (concerns, event tracking)
